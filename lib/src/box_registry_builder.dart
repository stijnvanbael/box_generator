  ...
  FutureOr<String> generateForAnnotatedElement(
    ...
    class $typeName\$BoxSupport extends EntitySupport<$typeName> {
      $typeName\$BoxSupport() : super(
        ...
        autogeneratedKeys: [${inspector.autogeneratedKeys.map((key) => "'${key.name}'").join(',')}],
...
class EntityInspector extends SimpleElementVisitor<void> {
  ...
  final List<FieldElement> autogeneratedKeys = [];
  ...
  void visitFieldElement(FieldElement element) {
    if (!element.isSynthetic &&
      ...
      fields.add(element);
      if (element.hasMeta(Key)) {
        keys.add(element);
        final key = element.getMeta<Key>();
        if (key?.getField('autogenerated')?.toBoolValue() ?? false) {
          autogeneratedKeys.add(element);
        }
      }
    }
  }
